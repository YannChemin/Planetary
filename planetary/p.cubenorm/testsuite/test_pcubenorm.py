...
ISIS3 equivalent: cubenorm
Purpose: Per-line/per-sample normalisation
...
class TestPcubenorm(TestCase):
    """Test p.cubenorm module.

    Verifies module interface and (where ISIS3 binary is available)
    cross-validates outputs against the equivalent ISIS3 application: cubenorm.
    """
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.cubenorm", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.cubenorm", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.cubenorm"),
                             "Module p.cubenorm not found in PATH")

    @unittest.skipUnless(shutil.which("cubenorm"),
                         "ISIS3 application cubenorm not available "
                         "in PATH - skipping cross-validation test")
    def test_isis3_equivalence(self):
        """Cross-validate against ISIS3 cubenorm when both are installed.
        ...
        # https://isis.astrogeology.usgs.gov/Application/presentation/Tabbed/cubenorm/cubenorm.html
