"""Test of p.atcorr.hapke

ISIS3 equivalent: (custom)
Purpose: Atmospheric correction (Hapke model)
...
class TestPatcorrhapke(TestCase):
    """Test p.atcorr.hapke module.
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.atcorr.hapke", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.atcorr.hapke", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.atcorr.hapke"),
                             "Module p.atcorr.hapke not found in PATH")
