"""Test of p.mineral.indices

ISIS3 equivalent: (custom)
Purpose: CRISM mineral indices
...
class TestPmineralindices(TestCase):
    """Test p.mineral.indices module.
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.mineral.indices", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.mineral.indices", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.mineral.indices"),
                             "Module p.mineral.indices not found in PATH")
