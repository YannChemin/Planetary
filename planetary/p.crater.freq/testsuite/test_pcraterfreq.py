"""Test of p.crater.freq

ISIS3 equivalent: (custom)
Purpose: Crater size-frequency dating
...
class TestPcraterfreq(TestCase):
    """Test p.crater.freq module.
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.crater.freq", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.crater.freq", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.crater.freq"),
                             "Module p.crater.freq not found in PATH")
