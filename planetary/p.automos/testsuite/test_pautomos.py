...
ISIS3 equivalent: automos
Purpose: Mosaic with feathering
...
class TestPautomos(TestCase):
    """Test p.automos module.

    Verifies module interface and (where ISIS3 binary is available)
    cross-validates outputs against the equivalent ISIS3 application: automos.
    """
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.automos", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.automos", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.automos"),
                             "Module p.automos not found in PATH")

    @unittest.skipUnless(shutil.which("automos"),
                         "ISIS3 application automos not available "
                         "in PATH - skipping cross-validation test")
    def test_isis3_equivalence(self):
        """Cross-validate against ISIS3 automos when both are installed.
        ...
        # https://isis.astrogeology.usgs.gov/Application/presentation/Tabbed/automos/automos.html
