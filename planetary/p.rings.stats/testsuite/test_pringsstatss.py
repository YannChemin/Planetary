"""Test of p.rings.stats

ISIS3 equivalent: (custom)
Purpose: Radial brightness statistics
...
class TestPringsstats(TestCase):
    """Test p.rings.stats module.
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.rings.stats", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.rings.stats", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.rings.stats"),
                             "Module p.rings.stats not found in PATH")
