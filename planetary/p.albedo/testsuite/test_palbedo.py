...
Purpose: I/F to geometric albedo
...
class TestPalbedo(TestCase):
    """Test p.albedo module.
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.albedo", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.albedo", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.albedo"),
                             "Module p.albedo not found in PATH")
