...
ISIS3 equivalent: bandnorm
Purpose: Multi-band normalisation
...
class TestPbandnorm(TestCase):
    """Test p.bandnorm module.

    Verifies module interface and (where ISIS3 binary is available)
    cross-validates outputs against the equivalent ISIS3 application: bandnorm.
    """
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.bandnorm", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.bandnorm", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.bandnorm"),
                             "Module p.bandnorm not found in PATH")

    @unittest.skipUnless(shutil.which("bandnorm"),
                         "ISIS3 application bandnorm not available "
                         "in PATH - skipping cross-validation test")
    def test_isis3_equivalence(self):
        """Cross-validate against ISIS3 bandnorm when both are installed.
        ...
        # https://isis.astrogeology.usgs.gov/Application/presentation/Tabbed/bandnorm/bandnorm.html
