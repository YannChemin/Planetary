...
ISIS3 equivalent: photomet
Purpose: Photometric normalisation
...
class TestPphotomet(TestCase):
    """Test p.photomet module.

    Verifies module interface and (where ISIS3 binary is available)
    cross-validates outputs against the equivalent ISIS3 application: photomet.
    """
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.photomet", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.photomet", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.photomet"),
                             "Module p.photomet not found in PATH")

    @unittest.skipUnless(shutil.which("photomet"),
                         "ISIS3 application photomet not available "
                         "in PATH - skipping cross-validation test")
    def test_isis3_equivalence(self):
        """Cross-validate against ISIS3 photomet when both are installed.
        ...
        # https://isis.astrogeology.usgs.gov/Application/presentation/Tabbed/photomet/photomet.html
