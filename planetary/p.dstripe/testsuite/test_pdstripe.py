...
ISIS3 equivalent: dstripe
Purpose: Detector destriping
...
class TestPdstripe(TestCase):
    """Test p.dstripe module.

    Verifies module interface and (where ISIS3 binary is available)
    cross-validates outputs against the equivalent ISIS3 application: dstripe.
    """
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.dstripe", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.dstripe", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.dstripe"),
                             "Module p.dstripe not found in PATH")

    @unittest.skipUnless(shutil.which("dstripe"),
                         "ISIS3 application dstripe not available "
                         "in PATH - skipping cross-validation test")
    def test_isis3_equivalence(self):
        """Cross-validate against ISIS3 dstripe when both are installed.
        ...
        # https://isis.astrogeology.usgs.gov/Application/presentation/Tabbed/dstripe/dstripe.html
