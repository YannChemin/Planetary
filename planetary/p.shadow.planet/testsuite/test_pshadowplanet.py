"""Test of p.shadow.planet

ISIS3 equivalent: shadow
Purpose: Shadow mask via ray-marching
...
class TestPshadowplanet(TestCase):
    """Test p.shadow.planet module.

    Verifies module interface and (where ISIS3 binary is available)
    cross-validates outputs against the equivalent ISIS3 application: shadow.
    """
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.shadow.planet", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.shadow.planet", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.shadow.planet"),
                             "Module p.shadow.planet not found in PATH")

    @unittest.skipUnless(shutil.which("shadow"),
                         "ISIS3 application shadow not available "
                         "in PATH - skipping cross-validation test")
    def test_isis3_equivalence(self):
        """Cross-validate against ISIS3 shadow when both are installed.
        ...
        # https://isis.astrogeology.usgs.gov/Application/presentation/Tabbed/shadow/shadow.html
