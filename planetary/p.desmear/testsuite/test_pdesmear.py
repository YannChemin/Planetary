...
ISIS3 equivalent: desmear
Purpose: TDI/push-broom desmearing
...
class TestPdesmear(TestCase):
    """Test p.desmear module.

    Verifies module interface and (where ISIS3 binary is available)
    cross-validates outputs against the equivalent ISIS3 application: desmear.
    """
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.desmear", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.desmear", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.desmear"),
                             "Module p.desmear not found in PATH")

    @unittest.skipUnless(shutil.which("desmear"),
                         "ISIS3 application desmear not available "
                         "in PATH - skipping cross-validation test")
    def test_isis3_equivalence(self):
        """Cross-validate against ISIS3 desmear when both are installed.
        ...
        # https://isis.astrogeology.usgs.gov/Application/presentation/Tabbed/desmear/desmear.html
