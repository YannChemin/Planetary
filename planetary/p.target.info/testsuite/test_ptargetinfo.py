"""Test of p.target.info

ISIS3 equivalent: (custom)
Purpose: SPICE planetary body info
...
class TestPtargetinfo(TestCase):
    """Test p.target.info module.
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.target.info", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.target.info", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.target.info"),
                             "Module p.target.info not found in PATH")
