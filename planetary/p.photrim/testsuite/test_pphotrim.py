...
ISIS3 equivalent: photrim
Purpose: Geometric mask via i/e/g angles
...
class TestPphotrim(TestCase):
    """Test p.photrim module.

    Verifies module interface and (where ISIS3 binary is available)
    cross-validates outputs against the equivalent ISIS3 application: photrim.
    """
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.photrim", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.photrim", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.photrim"),
                             "Module p.photrim not found in PATH")

    @unittest.skipUnless(shutil.which("photrim"),
                         "ISIS3 application photrim not available "
                         "in PATH - skipping cross-validation test")
    def test_isis3_equivalence(self):
        """Cross-validate against ISIS3 photrim when both are installed.
        ...
        # https://isis.astrogeology.usgs.gov/Application/presentation/Tabbed/photrim/photrim.html
