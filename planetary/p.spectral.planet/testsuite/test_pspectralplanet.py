"""Test of p.spectral.planet

ISIS3 equivalent: (custom)
Purpose: Spectral filtering operations
...
class TestPspectralplanet(TestCase):
    """Test p.spectral.planet module.
    ...
    def test_help(self):
        ...
        module = SimpleModule("p.spectral.planet", flags="h")
        self.assertModule(module)

    def test_module_metadata(self):
        """Test that module exposes proper GRASS metadata
        (keywords, label, description) via --interface-description."""
        module = SimpleModule("p.spectral.planet", flags="", overwrite=False,
                              run_=False)
        # Verify the module binary exists in PATH
        self.assertIsNotNone(shutil.which("p.spectral.planet"),
                             "Module p.spectral.planet not found in PATH")
